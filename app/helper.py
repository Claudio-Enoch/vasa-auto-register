...
def check_time_to_register() -> Tuple[str, str]:
    ...
    Classes will be returned starting 48 hours prior to the registration open.
    """
    ...
    for class_type, class_times in vasa_registration_json[offset_day].items():
        if time_now in class_times:
            print(f"Registration wanted for {class_type} at {time_now} on {offset_day}")
            return class_type, time_now  # ( "CARDIO", "9:30AM" )
    print(f"No registrations for {time_now} on {offset_day}")
    exit(0)
